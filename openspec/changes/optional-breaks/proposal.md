...
- Settings → Timer gains two enable/disable toggles — one above the Short Break duration slider, one above the Long Break duration slider. When a break type is disabled, its duration slider (and for long breaks, the Rounds until Long Break slider) is visually dimmed and non-interactive.
...
- **Frontend**: `src/messages/*.json` — add i18n keys for both toggles across all 8 locales.
